...
// Custom Icon for Agent (Red Color)
// --- UNIQUE CUSTOM AGENT ICON ---
const agentIcon = new L.Icon({
    // OPTION 1: Technician/Electrician with Helmet (Best for your project)
...
const ComplaintDetails = () => {
    ...
    useEffect(() => {
        const fetchDetails = async () => {
            ...
            try {
                const res = await axios.get(`${API_BASE_URL}/users/complaint/${ticketId}`, { 
    ...
    // 2. WebSocket Connection (Only if Agent is assigned)
    useEffect(() => {
        if (complaint && complaint.id && complaint.status !== 'Resolved') {
            const client = new Client({
                ...
                onConnect: () => {
                    ...
                    client.subscribe(topic, (message) => {
                        ...
                        setAgentPosition([loc.lat, loc.lng]);
                    });
                },
    ...
    const complaintPos = [complaint.latitude || 23.0225, complaint.longitude || 72.5714];

    return (
        <div className="page-content-card" style={{padding: '20px'}}>
            <h2>Details for Ticket: {complaint.ticketId}</h2>
            
            {/* --- MAP SECTION --- */}
            <div style={{ height: '400px', width: '100%', marginBottom: '20px', border: '2px solid #ddd', borderRadius: '10px' }}>
                <MapContainer center={complaintPos} zoom={13} style={{ height: '100%', width: '100%' }}>
                    <TileLayer url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png" />
                    
                    {/* Complaint Location (Default Blue) */}
                    <Marker position={complaintPos}>
            ...
            <div className="details-grid">
                ...
                {complaint.agent && (
                    <div style={{background: '#f9f9f9', padding: '10px', borderRadius: '5px'}}>
                        ...
                        <p>{complaint.agent.name} ({complaint.agent.mobileNumber || 'No Contact'})</p>
                        {!agentPosition && <small style={{color: 'gray'}}>Waiting for agent to start journey...</small>}
                    </div>
                )}
            </div>
        </div>
