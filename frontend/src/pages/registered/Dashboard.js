...
// Galti thi: title me 'verificationCode' use ho raha tha jo defined nahi tha. 'code' use karna tha.
const VerificationCodeModal = ({ code, ticketId, onClose }) => {
    return (
        <Modal isOpen={true} onClose={onClose} title={`Verification Code for ${ticketId}`}>
            <div style={{ textAlign: 'center', padding: '20px' }}>
                <p>Please share this 6-digit code with the visiting agent to confirm resolution.</p>
                <h1 style={{ color: '#28a745', fontSize: '3em', border: '2px dashed #28a745', padding: '10px', borderRadius: '5px', background: '#f9f9f9' }}>
                    {code}
                </h1>
                <p style={{ fontSize: '0.9em', color: 'red', marginTop: '10px' }}>
                    DO NOT share this code before the service is complete.
                </p>
                <button onClick={onClose} style={{ padding: '10px 20px', marginTop: '20px', backgroundColor: '#007bff', color: 'white', border: 'none', borderRadius: '5px', cursor: 'pointer' }}>
                    Close
                </button>
            </div>
        </Modal>
...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    fetchComplaints();
  }, [auth.token]); // Dependency updated to auth.token

  // --- 2. SSE Listener for OTP ---
  useEffect(() => {
        ...
        try {
            ...
            // Listen specifically for "verification_code"
            eventSource.addEventListener("verification_code", (event) => {
                const data = JSON.parse(event.data);
                console.log("🎉 OTP Received:", data);
                
                // State update karein taaki Modal khule
                setVerificationCode(data.verificationCode);
                setComplaintToVerify(data.ticketId);
                
                toast.success(`OTP Received for Ticket ${data.ticketId}`);
            });
    ...
    <div className="dashboard-container">
      <div className="dashboard-header">
        ...
        <p style={{ fontSize: '0.8em', color: 'gray' }}>
          Live Status: <span style={{color: sseStatus === 'CONNECTED' ? 'green' : 'red'}}>{sseStatus}</span>
        </p>
      </div>
      ...
      {/* --- 2. YAHAN HAI FIX: MODAL KO RENDER KARO --- */}
      {/* Agar verification code hai, tabhi modal dikhao */}
      {verificationCode && (
